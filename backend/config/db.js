...
const sequelize = new Sequelize(process.env.DATABASE_URL, {
  ...
  dialectOptions: {
    ...
    // Required for Supabase connection pooler (PgBouncer) on port 6543
...
const connectDB = async () => {
  try {
    ...
    const detail = error.message || (error.errors && error.errors[0]?.message) || error.code || String(error);
    const parent = error.parent?.message || error.original?.message || '';
    console.error("❌ Error connecting to database:", detail);
    if (parent) console.error("   Root cause:", parent);
    if (error.code) console.error("   Code:", error.code);
    console.warn("⚠️  Server will run WITHOUT database. DB-dependent routes will fail.");
    console.warn("   → Check that your Supabase project is not paused (free tier auto-pauses).");
    console.warn("   → Visit https://supabase.com/dashboard and click 'Restore project' if paused.");
  }
};
