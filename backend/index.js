...
app.get("/get-summarized-news/:category", async (req, res) => {
  ...
  try {
    const limit = parseInt(req.query.limit) || null;
    const forceLive = req.query.live === "1";
    console.log(`\nRetrieving ${limit ? limit : "all"} articles for category: ${category}. forceLive=${forceLive}`);

    let articles = [];
    if (!forceLive) {
      articles = await getArticlesByTopic(category, limit);
    }

    // Fallback: fetch live from SerpAPI if DB is empty or unavailable
    if (!articles || articles.length === 0) {
      console.log(`No DB articles for category "${category}" — fetching live from SerpAPI...`);
      ...
      const summarizedNews = await summarizeNewsArticles(articlesWithContent.slice(0, 8));
      // Try to save in background (won't crash if DB is down)
      try {
        const saveResult = await saveArticles(summarizedNews);
        console.log(`Auto-saved ${saveResult.count} (errors: ${saveResult.errors.length})`);
      } catch (saveErr) {
